...
tc <- textConnection(input)
pwords <- scan(tc, what = "", sep = "\n")
pwords %<>% str_split("[[:space:]]")

day4 <- function(pwords) {
  n_distinct(pwords) == length(pwords)
}

pwords %>% map_int(day4) %>% sum

day4_ii <- function(pwords) {
  pword_arrange <- pwords %>% 
    map_chr(~ .x %>% 
          strsplit("") %>% 
          unlist %>% 
          sort %>% 
  ...
  n_distinct(pword_arrange) == length(pword_arrange)
}

pwords %>% map_int(day4_ii) %>% sum
